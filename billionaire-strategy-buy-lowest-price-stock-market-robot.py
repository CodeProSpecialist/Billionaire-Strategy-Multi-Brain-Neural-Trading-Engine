...
if _KSB_AVAILABLE:
    ...
    # Minimum pretrain floor: no brain enters the live engines until it
    # has at least MIN_UPDATES_FLOOR synthetic examples under its belt.
    # Prevents "brain suite not loaded" — a cold brain with 0 updates
    # would ignore live signals and force the whole gate into shadow.
    MIN_UPDATES_FLOOR = 5000

    def _brain_n_updates(name):
        try:
            b = getattr(KSB, name, None)
    ...
    def _decide_pretrain_counts():
        """Return per-brain pretrain counts. If saved state loaded and
        the brain already has >= MIN_UPDATES_FLOOR, pretrain=0. Otherwise
        top up to MIN_UPDATES_FLOOR. First run tops every brain to 5k."""
        pairs = [
            ('regime',  'VOLATILITY_REGIME'),
            ('manager', 'MANAGER_BRAIN'),
            ('dt',      'DAYTRADE_BRAIN'),
            ('cap',     'CAPITAL_BRAIN'),
            ('pg',      'PROFIT_GROWTH_BRAIN'),
        ]
        counts = {}
        for short, attr in pairs:
            have = _brain_n_updates(attr)
            counts['pretrain_' + short] = max(0, MIN_UPDATES_FLOOR - have)
        return counts

    try:
        _ksb_had_state = _ksb_brain_state_exists()
        _pt = _decide_pretrain_counts()
        _topup = sum(_pt.values())
        if _ksb_had_state and _topup == 0:
            print(f"[merge] KSB: saved state loaded, all brains "
                  f"≥{MIN_UPDATES_FLOOR} updates — no pretrain needed")
        elif _ksb_had_state:
            print(f"[merge] KSB: saved state loaded, topping up "
                  f"{_topup} synth examples to reach {MIN_UPDATES_FLOOR}/brain")
        else:
            print(f"[merge] KSB: no saved state — pretraining "
                  f"{MIN_UPDATES_FLOOR} synth per brain")
        KSB.start_brain_suite(**_pt, start_engines=True)
        print(f"[merge] KSB: brain suite live "
              f"(floor={MIN_UPDATES_FLOOR}, top-up={_topup})")
