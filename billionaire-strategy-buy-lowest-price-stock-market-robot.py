...
def brain_suite_snapshot():
    """One call for the dashboard — every brain's state."""
    try:
        return {
            "volatility_regime": VOLATILITY_REGIME.snapshot(),
            "manager":           MANAGER_BRAIN.snapshot(),
            "capital_alloc":     CAPITAL_ALLOC.snapshot(),
            "profit_growth":     PROFIT_GROWTH.snapshot(),
            "risk_sentinel":     RISK_SENTINEL.snapshot(),
            "correlation_guard": CORRELATION_GUARD.snapshot(),
            "daytrade_brain":    DAYTRADE_BRAIN.snapshot(),
            "chandelier_brain":  CHANDELIER_BRAIN.snapshot(),
            "brain_comms":       BRAIN_COMMS.snapshot(),
            "trading_floor":     TRADING_FLOOR.snapshot(),
        }
    except Exception as e:
        return {"error": str(e)}


# ---------------- Nightly historical-trade replay -----------------------------
# Reads finished trades from TradeHistory, pairs buys with sells, reconstructs
# entry-time features from historical bars, and feeds each completed trade back
# through the brains as SUPERVISED learning. Runs once per day on a background
# thread; safe to call manually via brain_suite_replay_now().
_REPLAY_STATE_FILE = os.path.join(_BRAIN_DIR, "replay_state.json")
_replay_lock = _brain_threading.RLock()
...
def chair_decide(symbol, notional_requested, current_regime=None,
    ...
    Returns the same shape the old chair_decide did:
        {approved: bool, notional: float, reason: str, votes: [...]}
    so downstream code (buy_stocks) needs zero changes.
    """
    if not _KSB_AVAILABLE:
        # Fail-open (buy strategy is authoritative — brains are gates only).
        return {"approved": True, "notional": notional_requested,
                "reason": "KSB unavailable — fail-open",
                "votes": []}
    try:
        # Estimate strategy_score from ml_adjustment if provided, else 6.0.
        # The Alpaca strategy's raw score is not passed here; ml_adjustment
        # is a ±1.5 nudge on top of the score. Neutral 6.0 puts brain_p ~0.6.
                ...
                if row and (row.quantity - qty) > 1e-6:
                    ...
                    if feat_row and feat_row.entry_price:
                        ...
                        try:
                            ...
                            if _KSB_AVAILABLE:
                                try:
                                    _brain_p = 0.5  # placeholder — main bot's
                                    # original brain_p at entry not tracked
                                    # here; use neutral. The Manager still
                                    # learns from won/lost regardless.
                                    KSB.TRADING_FLOOR.on_trade_settled(
                                        symbol=symbol, won=bool(won),
                                        pnl=float(pnl_dollars),
                                        brain_p=_brain_p, side="LONG",
                                        indicators={"notional": float(
                                            getattr(position, "market_value",
                                                    100.0))})
                                except Exception as _e:
...
class DashboardWSEngine:
    ...
    def _build_state(self):
        ...
        # live per-position status the Kalshi UI shows
        try:
            pmdt = []
            for p in state.get('positions', []) or []:
                ...
                pmdt.append({
                    ...
                    "peak": p.get('high_water_mark') or current,
                    "state": "profit" if pnl_pct > 0 else ("loss" if pnl_pct < 0 else "flat"),
        ...
        state['settings'] = {
            ...
            'manager_strictness': MANAGER_BRAIN.strictness if MANAGER_BRAIN else 'medium',
        }
...
if _KSB_AVAILABLE:
    ...
    def _ksb_scheduler_loop():
        ...
        the process. First tick fires 24h after startup."""
        while True:
