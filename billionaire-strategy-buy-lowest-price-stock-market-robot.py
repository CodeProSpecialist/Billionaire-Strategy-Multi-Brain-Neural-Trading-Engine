...
#
# Feature vector (11 numbers, all clipped/normalized, derived from the SAME
# daily bars compute_buy_score already fetched -- no extra HTTP):
...
BRAIN_F_FEATURE_LABELS = [
    ...
    'vol_vs_sma20', 'sma20_gt_sma50',
]


def _brain_f_set_foundation_weights(model):
    """Hand-weighted 8-unit frozen foundation. Each unit fires on a specific
    ...
    W = np.zeros((BRAIN_F_FEATURES, 8), dtype=np.float32)
    b = np.zeros(8, dtype=np.float32)

    RSI, MACD, MACDH, R5, R20, D20, D50, SLOPE, ATRP, VOL, XUP = range(11)
...
def _brain_f_build_model(tf):
    ...
    x = keras.layers.Dense(8, activation='tanh', name='bullish_foundation',
                           use_bias=True)(inputs)
...
def _brain_f_synth_generate(n):
    ...
    for i in range(n):
        ...
        X[i] = [rsi, macd_n, macd_h, r5, r20, d20, d50, slope, atrp, vol, xup]
        # Ground-truth label: bullish + not overextended + not too choppy
        overextended = d20 > 0.15
        too_choppy = atrp > 0.05
        y[i] = 1.0 if (bullish and not overextended and not too_choppy) else 0.0
    return X, y


def brain_f_pretrain_if_needed():
    """Run once at startup. Idempotent: no-op if model already on disk."""
    if BRAIN_F_MODE == 'off':
        return
    ...
    print(f'[brain_f] no model on disk -- pretraining {BRAIN_F_PRETRAIN_SAMPLES:,} synthetic examples')
    t0 = time.time()
    ...
    if os.path.exists(BRAIN_F_MODEL_PATH):
        try:
            ...
            if _brain_f_model.input_shape and _brain_f_model.input_shape[-1] == BRAIN_F_FEATURES:
                return _brain_f_model
            logging.warning('brain_f: on-disk model feature-count mismatch; rebuilding')
            _brain_f_model = None
        except Exception as e:
            logging.warning(f'brain_f: load failed ({e}); rebuilding')
            _brain_f_model = None
    # No cached model -- build fresh (will use frozen foundation only, head untrained)
    _brain_f_model = _brain_f_build_model(tf)
    return _brain_f_model


def _brain_f_build_features_from_df(df, current_price=None):
    """Extract the 11-feature vector from a daily-bar DataFrame. Uses the
    SAME df compute_buy_score already fetched -- no extra HTTP. Returns
    None if the df is too short or missing required columns."""
    try:
        ...
        feats = np.array([rsi_norm, macd_norm, macd_hist_norm, r5, r20,
                          dist20, dist50, slope, atr_pct, vol_vs, xup],
                         dtype=np.float32)
...
def brain_f_score_symbol(symbol, df, current_price=None):
    ...
    if hit and (now - hit[0]) < BRAIN_F_INFER_CACHE_TTL:
        return hit[1]
    feats = _brain_f_build_features_from_df(df, current_price=current_price)
    if feats is None:
        return None
...
def main():
    ...
    # Brain F (bullish-trend picker) first-run pretraining. Same shape:
    # 20k synthetic examples, seconds on CPU. Feeds a small bounded bump
    # into buy_stocks scoring. No-op if model already exists on disk.
    try:
