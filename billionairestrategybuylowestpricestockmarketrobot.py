...
try:
    ...
    # ── Relocate brain persistence into the current working directory ──
    # KSB defaults its BRAIN_DIR to ~/.kalshi_style_brains (a hidden
    # folder under the user's home). That confused operators who expected
    # brain files next to the bot. Repoint every path to ./brains under
    # the bot's cwd BEFORE anything saves: update KSB.BRAIN_DIR + the
    # per-brain constants, reassign each brain singleton's .dir, migrate
    # any legacy state from the old home-dir location, and repoint the
    # trading-floor memory file.
    try:
        import os as _bd_os, shutil as _bd_shutil
        _NEW_BRAIN_DIR = _bd_os.path.join(_bd_os.getcwd(), "brains")
        _OLD_BRAIN_DIR = KSB.BRAIN_DIR
        _bd_os.makedirs(_NEW_BRAIN_DIR, exist_ok=True)
        # Migrate legacy brain state if present (one-shot; skipped after).
        if _OLD_BRAIN_DIR != _NEW_BRAIN_DIR and _bd_os.path.isdir(_OLD_BRAIN_DIR):
            for _entry in _bd_os.listdir(_OLD_BRAIN_DIR):
                _src = _bd_os.path.join(_OLD_BRAIN_DIR, _entry)
                _dst = _bd_os.path.join(_NEW_BRAIN_DIR, _entry)
                if _bd_os.path.exists(_dst):
                    continue   # never overwrite something already at the new home
                try:
                    _bd_shutil.move(_src, _dst)
                    print(f"[Brains] migrated {_entry}: "
                          f"{_OLD_BRAIN_DIR} → {_NEW_BRAIN_DIR}", flush=True)
                except Exception as _me:
                    print(f"[Brains] migrate {_entry}: {_me}", flush=True)
        # Repoint the KSB module-level constants.
        KSB.BRAIN_DIR = _NEW_BRAIN_DIR
        for _cname, _sub in (('REGIME_BRAIN_DIR',  'REGIME'),
                              ('MANAGER_BRAIN_DIR', 'MANAGER'),
                              ('DT_BRAIN_DIR',      'DAYTRADE'),
                              ('CAP_BRAIN_DIR',     'CAPITAL'),
                              ('PG_BRAIN_DIR',      'PROFIT_GROWTH'),
                              ('BRAIN_COMMS_DIR',   'BRAIN_COMMS')):
            _p = _bd_os.path.join(_NEW_BRAIN_DIR, _sub)
            _bd_os.makedirs(_p, exist_ok=True)
            setattr(KSB, _cname, _p)
        # Repoint each brain singleton's .dir onto the new location.
        for _attr, _sub in (('VOLATILITY_REGIME',   'REGIME'),
                             ('MANAGER_BRAIN',       'MANAGER'),
                             ('DAYTRADE_BRAIN',      'DAYTRADE'),
                             ('CAPITAL_BRAIN',       'CAPITAL'),
                             ('PROFIT_GROWTH_BRAIN', 'PROFIT_GROWTH')):
            _b = getattr(KSB, _attr, None)
            if _b is None: continue
            _new = _bd_os.path.join(_NEW_BRAIN_DIR, _sub)
            _b.dir = _new
            # Reload state from the new (post-migration) location so the
            # in-memory brain reflects whatever moved.
            if hasattr(_b, '_load_or_init'):
                try: _b._load_or_init()
                except Exception as _le:
                    print(f"[Brains] reload {_attr} at new dir: {_le}",
                          flush=True)
        # Repoint the trading-floor memory file too.
        try:
            _fm = getattr(KSB, 'FLOOR_MEMORY', None)
            if _fm is not None:
                _fm._path = _bd_os.path.join(_NEW_BRAIN_DIR,
                                              'trading_floor_memory.json')
                if hasattr(_fm, '_load'):
                    try: _fm._load()
                    except Exception: pass
            KSB.FLOOR_MEMORY_FILE = _bd_os.path.join(
                _NEW_BRAIN_DIR, 'trading_floor_memory.json')
        except Exception as _fe:
            print(f"[Brains] repoint FLOOR_MEMORY: {_fe}", flush=True)
        print(f"[Brains] persistence path set to {_NEW_BRAIN_DIR} "
              f"(was {_OLD_BRAIN_DIR})", flush=True)
    except Exception as _pe:
        print(f"[Brains] could not relocate brain dir: {_pe}", flush=True)
    ...
    def _ksb_shim():
        # BUGFIX: the previous shim did
        #   VOLATILITY_REGIME.__class__.regime = property(
        #       lambda self: self.snapshot().get('regime_class', 0))
        # which overrode the plain int `.regime` attribute (set in __init__)
        # with a property that calls snapshot() → which reads self.regime
        # → which calls snapshot() → infinite recursion, and because
        # snapshot() holds a non-reentrant Lock, every thread that ever
        # calls it deadlocks forever. This wedged main thread inside
        # build_all_brains and the every-10s status reporter alike.
        # KSB's VolatilityRegimeBrain already exposes `.regime` as an int
        # (0..3 = TRENDING/RANGING/CHOPPY/CRISIS) so nothing needs to be
        # shimmed here.
        ...
        # ── Trading-floor order engine shim ──
        # The KSB trading-floor terminal has a `buy <SYMBOL>` command
        # that fails with "no order_engine wired — call TRADING_FLOOR.
        # set_order_engine(...) from main bot" unless we hand it an
        # object exposing place_buy / buy / submit_market_buy AND a
        # sell hook. Wire a tiny shim that calls the bot's Alpaca
        # api.submit_order under the covers, so a user typing
        # `buy AAPL` in the dashboard floor terminal actually fires
        # a real market buy (still gated by unified_buy_gate + the
        # brains, so cold/blocked setups are refused).
        class _BotOrderEngine:
            def _submit(self, symbol, side, notional=None, qty=None):
                try:
                    if qty is None:
                        px = 0.0
                        try:
                            px = float(KSB.get_calibrated_price(KSB.BUS, symbol))
                        except Exception: pass
                        if not px or px <= 0:
                            try:
                                # Best-effort last-quote fetch from Alpaca.
                                q = api.get_latest_trade(symbol)
                                px = float(getattr(q, 'price', 0) or 0)
                            except Exception: pass
                        if not px or px <= 0:
                            return {"ok": False, "error": f"no price for {symbol}"}
                        qty = max(1, int(float(notional or 0) // px))
                    o = api.submit_order(
                        symbol=symbol, qty=str(int(qty)), side=side,
                        type='market', time_in_force='day')
                    return {"ok": True, "id": getattr(o, 'id', ''),
                            "qty": int(qty), "side": side}
                except Exception as e:
                    return {"ok": False, "error": str(e)}
            def place_buy(self, symbol, notional):
                return self._submit(symbol, 'buy', notional=notional)
            def buy(self, symbol, notional):
                return self.place_buy(symbol, notional)
            def submit_market_buy(self, symbol, notional):
                return self.place_buy(symbol, notional)
            def place_sell(self, symbol, qty=None):
                return self._submit(symbol, 'sell', qty=qty)
            def sell(self, symbol, qty=None):
                return self.place_sell(symbol, qty=qty)
            def submit_market_sell(self, symbol, qty=None):
                return self.place_sell(symbol, qty=qty)

        try:
            _bot_oe = _BotOrderEngine()
            if hasattr(TRADING_FLOOR, 'set_order_engine'):
                TRADING_FLOOR.set_order_engine(_bot_oe)
                print("[merge] KSB trading floor: order_engine wired "
                      "(place_buy/place_sell → Alpaca api.submit_order)",
                      flush=True)
            else:
                # Older KSB — attach as attribute so _cmd_buy sees it.
                TRADING_FLOOR._order_engine = _bot_oe
                print("[merge] KSB trading floor: order_engine attached "
                      "via _order_engine (no set_order_engine method)",
                      flush=True)
        except Exception as _oe:
            print(f"[merge] KSB trading floor: order_engine wire failed: {_oe}",
                  flush=True)
    _ksb_shim()
...
def brain_suite_snapshot():
    ...
    try:
        ...
        try:
            ...
            tf = {"error": str(_tfe)}
        # Live startup progress bars (phase + updates/target/min per brain).
        try:
            _tp = brain_training_progress_snapshot()
        except Exception:
            _tp = {}
        return {
            ...
            "brain_comms":       bc,
            "trading_floor":     tf,
            "training_progress": _tp,
            "min_updates":       globals().get('MIN_ACCEPTABLE_UPDATES', 3000),
            "target_updates":    globals().get('PRETRAIN_TARGET_UPDATES', 5000),
        }
    except Exception as e:
...
_BRAINS_TRAINING_NOTICE_TS = 0.0
_BRAINS_TRAINING_NOTICE_INTERVAL = 60.0  # seconds between floor notices

def _brains_still_training():
    """True while any KSB brain hasn't reached the minimum-updates floor.
    Uses the same MIN_UPDATES_FLOOR the pretrain orchestrator uses so the
    'ready' definition stays consistent across the file."""
    if not _KSB_AVAILABLE:
        return False
    try:
        floor = globals().get('MIN_UPDATES_FLOOR', 5000)
        for attr in ('VOLATILITY_REGIME', 'MANAGER_BRAIN', 'DAYTRADE_BRAIN',
                     'CAPITAL_ALLOC', 'PROFIT_GROWTH'):
            b = getattr(KSB, attr, None)
            if b is None:
                continue
            snap = b.snapshot() if hasattr(b, 'snapshot') else {}
            if int(snap.get('n_updates', 0) or 0) < floor:
                return True
        return False
    except Exception:
        return False

def _floor_notice_brains_training(symbol):
    """Post a throttled notice to the brain trading floor explaining that
    trades are being decided by technical indicators while the brains
    finish training."""
    global _BRAINS_TRAINING_NOTICE_TS
    now = time.time()
    msg = (f"{symbol}: brains still training — using technical indicators "
           f"for this decision (brain votes disabled until training completes)")
    try:
        if now - _BRAINS_TRAINING_NOTICE_TS >= _BRAINS_TRAINING_NOTICE_INTERVAL:
            _BRAINS_TRAINING_NOTICE_TS = now
            floor_post('CHAIR', 'training', msg)
            try:
                KSB.BRAIN_COMMS.post("CHAIR", "TRAINING", msg,
                                     {"symbol": symbol})
            except Exception:
                pass
    except Exception:
...
def chair_decide(symbol, notional_requested, current_regime=None,
    ...
    if not _KSB_AVAILABLE:
        return {"approved": True, "notional": notional_requested,
                ...
                "multiplier": 1.0}
    # While brains are still training, bypass the KSB gate entirely and
    # rely on the technical-indicator strategy score for the buy decision.
    # Post a throttled notice to the brain trading floor so operators see
    # that trades are being decided by TA, not by the (still-training) brains.
    if _brains_still_training():
        _floor_notice_brains_training(symbol)
        return {"approved": True, "notional": notional_requested,
                "reason": "brains still training — using technical indicators",
...
# Downstream sector filter (_SCANNER_EXCLUDED_SECTORS) removes the
# unwanted sectors; small-cap / unknown-sector names get filtered by
# the yfinance sector lookup + market-cap threshold in the scanner
# itself. We keep the full 500 here so the scanner has the whole
# large-cap universe to rank against.
...
# Small-cap threshold — anything under this market cap is dropped by
# the scanner. $2B is the standard small-cap upper bound; S&P 500 is
# almost entirely mid+large, so this only trims the smallest names.
_SCANNER_MIN_MARKET_CAP = 2_000_000_000


@sleep_and_retry
@limits(calls=120, period=60)
def _scanner_fetch_sector_and_cap(symbol):
    """Fetch sector + market cap in one yfinance call so we can filter
    small caps and unknown sectors without a second round trip."""
    try:
        ticker = yf.Ticker(symbol)
        info = ticker.info or {}
        ...
        logging.warning(f"scanner: fetch_sector_and_cap({symbol}) failed: {e}")
        return ('Unknown', 0.0)
...
def run_stock_scanner(show_progress=False):
    ...
    with _scanner_futures.ThreadPoolExecutor(max_workers=10) as ex:
        sec_futs = {ex.submit(_scanner_fetch_sector_and_cap, sc['symbol']): sc
                    for sc in scores}
        for f in _scanner_futures.as_completed(sec_futs):
            sc = sec_futs[f]
            try:
                sector, mcap = f.result()
                sc['sector']      = sector
                sc['market_cap']  = mcap
            except Exception:
                sc['sector']      = 'Unknown'
    ...
    # Drop small caps and unknown-sector names BEFORE sector top-25 groupby
    # so the scanner ranks a clean large/mid-cap universe.
    _before = len(df)
    df = df[df['sector'] != 'Unknown']
    df = df[df['market_cap'] >= _SCANNER_MIN_MARKET_CAP]
    logging.info(f"scanner: filtered {_before - len(df)} small-cap/unknown-sector "
                 f"names; {len(df)} remain (min market cap "
                 f"${_SCANNER_MIN_MARKET_CAP/1e9:.1f}B)")
...
def buy_stocks(symbols_to_buy_list, lock):
    ...
    _DECISION_IN_PROGRESS.set()
    try:
        return _buy_stocks_body(symbols_to_buy_list, lock)
    finally:
        _DECISION_IN_PROGRESS.clear()

def _buy_stocks_body(symbols_to_buy_list, lock):
...
def sell_stocks(lock):
    _DECISION_IN_PROGRESS.set()
    try:
        return _sell_stocks_body(lock)
    finally:
        _DECISION_IN_PROGRESS.clear()

def _sell_stocks_body(lock):
...
# Decision-window flag: buy_stocks / sell_stocks set this while they're
# actively making trade decisions. The KSB retrain scheduler and the
# manual retrain path check it and defer training so decision paths
# aren't contending for the same KSB lock.
_DECISION_IN_PROGRESS = threading.Event()

def _decision_window(label='cycle'):
    """Context-manager style: use in a `with` block around any buy/sell
    decision loop so retrain gets deferred."""
    class _CM:
        def __enter__(self_): _DECISION_IN_PROGRESS.set(); return self_
        def __exit__(self_, *a): _DECISION_IN_PROGRESS.clear()
    return _CM()
    ...
    def _apply_setting(self, key, value):
        ...
        try:
            if key == 'ksb_gate_mode' and value in ('off', 'shadow', 'armed'):
                ...
                # Recreate every brain dir (in case something wiped them),
                # rebuild each brain's model, then run the 24h scheduler
                # tick immediately (async — don't block the dashboard).
                if not _KSB_AVAILABLE:
                ...
                try:
                    ...
                    def _retrain_now():
                        try: _ensure_brain_dirs()
                        except Exception as _de:
                            floor_post('BOT', 'info', f"retrain dir prep: {_de}")
                        try: build_all_brains()
                        except Exception as _be:
                            floor_post('BOT', 'info', f"retrain build: {_be}")
                        _ksb_retrain_all_from_replay()
                        floor_post('BOT', 'info',
                                    "KSB retrain pass complete")
                    _thr.Thread(target=_retrain_now,
                ...
                # Wipe the on-disk brain state, recreate the directories +
                # models, then pretrain each brain to 20k synthetic updates
                # with live per-brain progress bars on the dashboard. Async
                ...
                try:
                    ...
                    def _fresh():
                        try: _ensure_brain_dirs()
                            ...
                            floor_post('BOT', 'info', f"pretrain dir prep: {_de}")
                        # Rebuild each brain's model (fresh weights).
                        try:
                            for _s, _a, _sub in BRAIN_ROSTER:
                                _b = getattr(KSB, _a, None)
                                if _b is None: continue
                                _b.model = None
                                if hasattr(_b, '_load_or_init'):
                                    try: _b._load_or_init()
                                        ...
                                        floor_post('BOT', 'info',
                                                    f"rebuild {_s}: {_le}")
                                # Zero cumulative counters so the pretrainer
                                # actually retrains up to the fresh target.
                                for _fld in ('n_updates', 'last_train_n',
                                              'wins', 'losses'):
                                    if hasattr(_b, _fld):
                                        setattr(_b, _fld, 0)
                        except Exception as _re:
                            floor_post('BOT', 'info', f"pretrain build: {_re}")
                        # Pretrain each brain to the max acceptable target.
                        _target = MAX_ACCEPTABLE_UPDATES
                        for _s, _a, _sub in BRAIN_ROSTER:
                            try: run_synthetic_pretrainer(_s, _target)
                            except Exception as _pe:
                                floor_post('BOT', 'info',
                                            f"pretrain {_s}: {_pe}")
                        try: reload_all_brains()
                        except Exception: pass
                        floor_post('BOT', 'info',
                                    f"KSB fresh pretrain done "
                                    f"({_target:,}×{len(BRAIN_ROSTER)})")
                    _thr.Thread(target=_fresh, daemon=True,
                                name='KSB-fresh-pretrain').start()
                    floor_post('USER', 'instruction',
                                f"KSB fresh pretrain queued "
                                f"({MAX_ACCEPTABLE_UPDATES:,}×{len(BRAIN_ROSTER)})")
                except Exception as _e:
...
# Mirrors the kalshi day-trading robot's brain creation pattern:
#   1. build_all_brains()          — construct every model up front
#   2. brains_needing_training()   — check which are below the pretrain floor
#   3. run_synthetic_pretrainer()  — pretrain each one that needs it
#   4. reload_all_brains()         — reload from disk into the live process
#   5. summary log + engines start + 24h retrain scheduler
# On subsequent runs, brains load their saved model.keras and skip pretrain.
if _KSB_AVAILABLE:
    import os as _ksb_os, threading as _ksb_threading, time as _ksb_time

    # ── Roster: (short_name, KSB module attribute, on-disk subdir) ──
    # Mirrors kalshi's BRAIN_COINS list — every entry here gets built,
    # trained if needed, reloaded, and reported in the readiness summary.
    BRAIN_ROSTER = [
        ('regime',  'VOLATILITY_REGIME',   'REGIME'),
        ('manager', 'MANAGER_BRAIN',       'MANAGER'),
        ('dt',      'DAYTRADE_BRAIN',      'DAYTRADE'),
        ('cap',     'CAPITAL_BRAIN',       'CAPITAL'),
        ('pg',      'PROFIT_GROWTH_BRAIN', 'PROFIT_GROWTH'),
    ]

    # Kalshi ported this: `BRAIN_COINS = ["BTC","DOGE","SOL",…]`. The
    # equivalent for a NASDAQ day-trader is the full NASDAQ 100 index
    # membership — the universe brains train + score against. Seeded
    # into KSB.set_brain_symbols() at startup. NOTE: KSB internally
    # caps feature aggregation at BRAIN_SYMBOLS_MAX (=8) to keep the
    # feature vector fixed-width; the rest of the roster is used for
    # per-symbol scoring, the daytrade brain, correlation guard, etc.
    # List reflects NASDAQ-100 constituents as of 2026-01; refresh
    # from https://www.nasdaq.com/market-activity/quotes/nasdaq-ndx-index
    # when Nasdaq rebalances (usually annual, third Friday of December).
    NASDAQ_BRAIN_SYMBOLS = [
        "AAPL", "MSFT", "NVDA", "AMZN", "META", "GOOGL", "GOOG", "AVGO",
        "TSLA", "COST", "NFLX", "TMUS", "AMD", "PEP", "CSCO", "LIN",
        "ADBE", "QCOM", "INTU", "TXN", "AMGN", "ISRG", "AMAT", "HON",
        "BKNG", "PANW", "VRTX", "CMCSA", "ADP", "SBUX", "GILD", "ADI",
        "MU", "MELI", "INTC", "LRCX", "REGN", "PDD", "KLAC", "PYPL",
        "MDLZ", "SNPS", "CDNS", "CRWD", "CTAS", "MAR", "ORLY", "MRVL",
        "ASML", "MNST", "FTNT", "ABNB", "CEG", "ADSK", "WDAY", "PCAR",
        "CHTR", "DASH", "ROP", "NXPI", "FANG", "PAYX", "AEP", "KDP",
        "CPRT", "ROST", "ODFL", "AZN", "FAST", "BKR", "TTD", "DDOG",
        "EA", "GEHC", "KHC", "VRSK", "CTSH", "EXC", "LULU", "IDXX",
        "XEL", "CSGP", "ON", "CCEP", "TEAM", "DXCM", "MCHP", "ANSS",
        "ZS", "TTWO", "CDW", "BIIB", "GFS", "WBD", "ILMN", "MDB",
        "ARM", "MRNA", "DLTR", "SMCI", "SIRI",
    ]

    # Every brain must reach at least MIN_ACCEPTABLE_UPDATES synthetic
    # updates before the main trading loop is allowed to start — main()
    # blocks on init_and_pretrain_all_brains() until this floor is hit
    # for every roster brain. PRETRAIN_TARGET_UPDATES is the training
    # target (clamped into [MIN_ACCEPTABLE_UPDATES, MAX_ACCEPTABLE_UPDATES]).
    MIN_ACCEPTABLE_UPDATES = 3000
    MAX_ACCEPTABLE_UPDATES = 20000
    PRETRAIN_TARGET_UPDATES = 20000
    MIN_UPDATES_FLOOR = PRETRAIN_TARGET_UPDATES   # legacy alias

    # ── Live per-brain training progress (read by the dashboard) ──
    #   phase   : 'pending' | 'building' | 'training' | 'done' | 'error'
    #   updates : current n_updates on this brain
    #   target  : synthetic-update target for this run
    #   min     : hard floor the main loop waits for
    #   elapsed : seconds spent in the current phase
    #   error   : last error string (only when phase=='error')
    BRAIN_TRAINING_PROGRESS = {
        short: {'phase': 'pending', 'updates': 0,
                'target': PRETRAIN_TARGET_UPDATES,
                'min':    MIN_ACCEPTABLE_UPDATES,
                'elapsed': 0.0, 'error': ''}
        for short, _, _ in BRAIN_ROSTER
    }
    _BRAIN_PROGRESS_LOCK = _ksb_threading.Lock()

    def _set_brain_progress(short, **fields):
        with _BRAIN_PROGRESS_LOCK:
            row = BRAIN_TRAINING_PROGRESS.setdefault(short, {})
            row.update(fields)

    def brain_training_progress_snapshot():
        """Thread-safe copy for the WS dashboard payload."""
        with _BRAIN_PROGRESS_LOCK:
            return {k: dict(v) for k, v in BRAIN_TRAINING_PROGRESS.items()}

    def _ksb_brain_state_exists():
        """True if ANY roster brain has a saved model.keras on disk. If none
    ...
    def _ensure_brain_dirs():
        """Belt-and-suspenders: every roster brain's on-disk subdir must
        exist. Never raises — logs each failure with the target path so
        the crash detector's tail names the exact directory that broke."""
        # KSB.BRAIN_DIR itself may be missing (KSB attr) or unwritable.
        base = None
        try:
            base = getattr(KSB, 'BRAIN_DIR', None)
        except Exception as e:
            _log_step("dirs", "get_base.error", str(e))
            return
        if not base:
            base = _ksb_os.path.join(_ksb_os.getcwd(), "brains")
            _log_step("dirs", "fallback_base", base)
            try: setattr(KSB, 'BRAIN_DIR', base)
            except Exception: pass
        try:
            _ksb_os.makedirs(base, exist_ok=True)
            _log_step("dirs", "base.ok", base)
        except Exception as e:
            _log_step("dirs", "base.error", f"{base}: {e}")
        for _short, attr, sub in BRAIN_ROSTER:
            b = None
            try: b = getattr(KSB, attr, None)
            except Exception as e:
                _log_step(_short, "getattr.error", f"{attr}: {e}")
            dirs = []
            try:
                if b is not None and getattr(b, 'dir', None):
                    dirs.append(b.dir)
            except Exception: pass
            try: dirs.append(_ksb_os.path.join(base, sub))
            except Exception as e:
                _log_step(_short, "join.error", f"{sub}: {e}")
            for d in dirs:
                try:
                    _ksb_os.makedirs(d, exist_ok=True)
                    _log_step(_short, "mkdir.ok", d)
                except Exception as e:
                    _log_step(_short, "mkdir.error", f"{d}: {e}")

    def _brain_model_file_exists(sub):
        """True iff the brain's model.keras is on disk. Used to detect
        partial saves (meta.json wrote but model didn't) that would
        otherwise fool brains_needing_training into thinking a brain is
        already trained when its weights are gone."""
        try:
            return _ksb_os.path.exists(
                _ksb_os.path.join(KSB.BRAIN_DIR, sub, "model.keras"))
        except Exception:
            return False

    def build_all_brains():
        """Construct every roster brain up front. Never raises — each
        brain is fully isolated in try/except so one broken brain can't
        stop the others, and every phase writes a _log_step line so the
        crash detector's tail names exactly where a failure happened."""
        try: _ensure_brain_dirs()
        except Exception as e:
            import traceback
            _log_step("dirs", "ensure.error", str(e))
            traceback.print_exc()
        built = 0
        for short, attr, _sub in BRAIN_ROSTER:
            _t0 = _ksb_time.time()
            _log_step(short, "build.start", attr)
            try:
                _set_brain_progress(short, phase='building', updates=0,
                                    elapsed=0.0, error='')
            except Exception as e:
                _log_step(short, "build.progress.error", str(e))
            b = None
            try: b = getattr(KSB, attr, None)
            except Exception as e:
                _log_step(short, "build.getattr.error", str(e))
            if b is None:
                _log_step(short, "build.missing", f"{attr} not in KSB")
                try:
                    _set_brain_progress(short, phase='error',
                                        error=f'{attr} not in KSB',
                                        elapsed=_ksb_time.time() - _t0)
                except Exception: pass
                continue
            try:
                if getattr(b, 'model', None) is None and hasattr(b, '_load_or_init'):
                    _log_step(short, "build.load_or_init", "start")
                    b._load_or_init()
                    _log_step(short, "build.load_or_init", "ok")
                else:
                    _log_step(short, "build.model.exists",
                              f"model={type(getattr(b,'model',None)).__name__}")
                built += 1
                try:
                    _set_brain_progress(short, phase='built',
                                        updates=_brain_n_updates(attr),
                                        elapsed=_ksb_time.time() - _t0)
                except Exception: pass
                _log_step(short, "build.ok", "")
            except Exception as e:
                import traceback
                _log_step(short, "build.error", str(e))
                traceback.print_exc()
                try:
                    _set_brain_progress(short, phase='error', error=str(e),
                                        elapsed=_ksb_time.time() - _t0)
                except Exception: pass
        _log_step("BOOT", "build_all_brains.return", f"{built}/{len(BRAIN_ROSTER)}")
        return built

    def _brain_n_updates(attr):
        """Return the brain's cumulative n_updates (0 if missing or errored)."""
        try:
            b = getattr(KSB, attr, None)
            if b is None: return 0
            snap = b.snapshot() if hasattr(b, 'snapshot') else {}
            return int(snap.get('n_updates', 0) or 0)
        except Exception:
            return 0

    def brains_needing_training(target_updates=PRETRAIN_TARGET_UPDATES):
        """Kalshi mirror: return the shorts of every roster brain whose
        cumulative n_updates < target_updates OR whose model.keras is
        missing from disk (partial save from a prior run). Cheap — reads
        each brain's snapshot() and one os.path.exists per brain."""
        todo = []
        for short, attr, sub in BRAIN_ROSTER:
            has_model = _brain_model_file_exists(sub)
            if not has_model or _brain_n_updates(attr) < target_updates:
                todo.append(short)
        return todo

    def _post_training_progress(short, attr, updates, target, elapsed):
        """Post a training-% message to the Brain Comms trading floor so
        the dashboard's Brain Comms Trading Floor panel shows live pretrain
        progress right alongside the rest of the brain chatter."""
        pct = int(min(100, (updates / max(1, target)) * 100))
        who = f"{attr}"
        msg = (f"pretraining {pct}% — {updates:,}/{target:,} updates "
               f"({elapsed:.1f}s)")
        try:
            if hasattr(KSB, 'TRADING_FLOOR') and hasattr(KSB.TRADING_FLOOR, 'training'):
                KSB.TRADING_FLOOR.training(who, msg)
        except Exception: pass
        try:
            if hasattr(KSB, 'BRAIN_COMMS'):
                KSB.BRAIN_COMMS.post(who, "TRAIN", msg,
                                      {"pct": pct, "updates": updates,
                                       "target": target, "elapsed": elapsed})
        except Exception: pass
        try:
            if hasattr(KSB, 'FLOOR_MEMORY'):
                KSB.FLOOR_MEMORY.add('training_events',
                                      {'brain': who, 'detail': msg,
                                       'pct': pct})
        except Exception: pass

    # ── Step-by-step training log + startup crash detector ────────────
    # Every phase of build/pretrain/save is written to this rolling log
    # AND stdout. On any crash, _startup_crash_check() prints a big
    # diagnostic banner naming the last step that succeeded before the
    # exception so the operator can see exactly where init died.
    _STARTUP_LOG = []                          # list of (ts, brain, step, msg)
    _STARTUP_LOG_LOCK = _ksb_threading.Lock()
    _STARTUP_LOG_FILE = _ksb_os.path.join(
        _ksb_os.getcwd(), "brains", "startup_train.log")

    def _log_step(brain, step, msg=""):
        """Append one line to the startup training log. Prints to stdout
        (flushed) AND to brains/startup_train.log. Never raises."""
        try:
            ts = _ksb_time.strftime("%H:%M:%S")
            line = f"[{ts}][{brain}][{step}] {msg}".rstrip()
            with _STARTUP_LOG_LOCK:
                _STARTUP_LOG.append((ts, brain, step, msg))
            print(f"  · {line}", flush=True)
            try:
                _ksb_os.makedirs(_ksb_os.path.dirname(_STARTUP_LOG_FILE),
                                  exist_ok=True)
                with open(_STARTUP_LOG_FILE, "a") as _lf:
                    _lf.write(line + "\n")
            except Exception: pass
        except Exception: pass

    _PYSPY_LOG_FILE = _ksb_os.path.join(_ksb_os.getcwd(), "brains",
                                          "startup_pyspy.log")

    def _inproc_thread_dump():
        """Fallback for when py-spy can't attach: use stdlib to dump
        every live thread's Python stack from inside the process. No
        permissions needed. Format mirrors py-spy dump output closely
        so downstream tooling / eyeballs can read either interchangeably."""
        import sys, threading, traceback
        lines = [f"Process {_ksb_os.getpid()}: {' '.join(sys.argv)}"]
        ...
        for tid, frame in frames.items():
            ...
            lines.append("")
            lines.append(f"Thread {tid} (active): \"{name}\"{daemon}")
            stack = traceback.extract_stack(frame)
            for f in reversed(stack):
                fn = f.filename
                lines.append(f"    {f.name} ({fn}:{f.lineno})")
        return "\n".join(lines)

    def _dump_py_spy(tag="startup"):
        """Write a full thread-stack dump to ./brains/startup_pyspy.log
        and echo the first 40 lines to the terminal. Tries `py-spy dump`
        first (external tool, richest output); if py-spy is missing or
        PTRACE is denied, falls back to an in-process stdlib dump so a
        stack trace is ALWAYS produced. Never raises."""
        import subprocess, shutil
        _log_step("py-spy", f"{tag}.start", f"pid={_ksb_os.getpid()}")
        out = None
        source = None
        ...
        if exe:
            try:
                p = subprocess.run(
                    [exe, "dump", "--pid", str(_ksb_os.getpid())],
                    capture_output=True, text=True, timeout=30)
                # py-spy prints stacks on stdout; PTRACE denial writes a
                # short "Permission Denied" to stderr with rc≠0.
                ...
                _log_step("py-spy", f"{tag}.spawn.error", str(e))
        else:
            _log_step("py-spy", f"{tag}.missing",
                      "py-spy not on PATH — using in-process fallback")
        if out is None:
            try:
                out = _inproc_thread_dump()
                source = "inproc-fallback"
            except Exception as e:
                _log_step("py-spy", f"{tag}.inproc.error", str(e))
                return
        try:
            _ksb_os.makedirs(_ksb_os.path.dirname(_PYSPY_LOG_FILE),
                              exist_ok=True)
            ts = _ksb_time.strftime("%Y-%m-%d %H:%M:%S")
            with open(_PYSPY_LOG_FILE, "a") as f:
                f.write(f"\n{'═'*70}\n{ts}  thread dump [{source}] "
                        f"({tag})  pid={_ksb_os.getpid()}\n"
                        f"{'═'*70}\n{out}\n")
        except Exception as e:
            _log_step("py-spy", f"{tag}.write.error", str(e))
            return
        head = out.splitlines()[:40]
        print(f"[thread-dump:{source}:{tag}] → {_PYSPY_LOG_FILE}",
              flush=True)
        for ln in head:
            print(f"  {ln}", flush=True)
        if len(out.splitlines()) > 40:
            print(f"  … ({len(out.splitlines())-40} more lines in file)",
                  flush=True)
        _log_step("py-spy", f"{tag}.ok",
                  f"[{source}] {len(out.splitlines())} lines")

    def _startup_crash_check(exc, phase):
        """Called from init_and_pretrain_all_brains's outer except. Prints
        a big banner naming the crashing phase + the last few good steps
        so the operator can immediately diagnose a startup failure."""
        import traceback
        print("\n" + "█" * 70, flush=True)
        print(f"█  STARTUP CRASH DETECTED — phase: {phase}", flush=True)
        print(f"█  exception: {type(exc).__name__}: {exc}", flush=True)
        print("█" * 70, flush=True)
        traceback.print_exc()
        print("─" * 70, flush=True)
        print("  Last steps before the crash:", flush=True)
        with _STARTUP_LOG_LOCK:
            tail = list(_STARTUP_LOG)[-15:]
        for ts, brain, step, msg in tail:
            print(f"    [{ts}][{brain}][{step}] {msg}", flush=True)
        print("─" * 70, flush=True)
        print(f"  Full log: {_STARTUP_LOG_FILE}", flush=True)
        print("█" * 70 + "\n", flush=True)

    def run_synthetic_pretrainer(short_name,
                                  target_updates=PRETRAIN_TARGET_UPDATES):
        """Kalshi mirror: pretrain ONE brain (identified by roster short name)
        with synthetic data until it reaches target_updates. Delegates to the
        brain's own pretrain(n) and runs a small poller thread on the side
        so the dashboard progress bar advances live as n_updates climbs."""
        target_updates = max(MIN_ACCEPTABLE_UPDATES,
                             min(MAX_ACCEPTABLE_UPDATES, int(target_updates)))
        entry = next((e for e in BRAIN_ROSTER if e[0] == short_name), None)
        if entry is None:
            print(f"[PreTrainer] unknown brain '{short_name}'")
            return False
        _, attr, _ = entry
        b = getattr(KSB, attr, None)
        if b is None or not hasattr(b, 'pretrain'):
            _set_brain_progress(short_name, phase='error',
                                error='no pretrain()', target=target_updates)
            print(f"[PreTrainer:{short_name}] no pretrain() — skipped")
            return False
        have = _brain_n_updates(attr)
        _set_brain_progress(short_name, phase='training', updates=have,
                            target=target_updates, elapsed=0.0, error='')
        if have >= target_updates:
            _set_brain_progress(short_name, phase='done', updates=have,
                                target=target_updates, elapsed=0.0)
            print(f"[PreTrainer:{short_name}] already at {have:,} updates "
                  f"(≥{target_updates:,}) — skipped")
            return True
        _t0 = _ksb_time.time()
        # Belt-and-suspenders: create the on-disk subdir BEFORE pretrain
        # so the save at the end never hits "[Errno 2] No such file or
        # directory: '.../REGIME/model.keras'".
        try:
            if getattr(b, 'dir', None):
                _ksb_os.makedirs(b.dir, exist_ok=True)
        except Exception as _de:
            print(f"[PreTrainer:{short_name}] makedirs {getattr(b,'dir','?')}: {_de}",
                  flush=True)
        print(f"[PreTrainer:{short_name}] target {target_updates:,} updates "
              f"(have {have:,}, need +{target_updates - have:,})…", flush=True)

        # Banner block copied from kalshi's run_synthetic_pretrainer so
        # the operator sees the exact same startup log they used to
        # (target, param count, then the periodic rate/ETA/WR/buffer line).
        _bname = getattr(b, '__class__', type(b)).__name__
        _params = 0
        try: _params = int(b.model.count_params()) if getattr(b, 'model', None) else 0
        except Exception: _params = 0
        print("=" * 70, flush=True)
        print(f"  SYNTHETIC PRE-TRAINER [{_bname}] "
              f"-- target: {target_updates:,} updates", flush=True)
        print(f"  Model params: ~{_params:,}", flush=True)
        print("=" * 70, flush=True)

        _post_training_progress(short_name, attr, have, target_updates, 0.0)

        # Side-poller: KSB's pretrain() is one blocking model.fit(); we
        # sample n_updates every 500ms so the dashboard progress bar moves
        # AND we emit the kalshi-style progress log every ~5,000 updates
        # (or 10% steps for smaller targets) with rate + ETA + WR + buffer.
        _stop = _ksb_threading.Event()
        _last_log_at = {'v': 0}   # last n_updates value we logged
        _LOG_EVERY = 5000
        _MIN_STEP  = max(1, target_updates // 10)
        def _pump():
            while not _stop.is_set():
                _upd = _brain_n_updates(attr)
                _el  = _ksb_time.time() - _t0
                _set_brain_progress(short_name, updates=_upd, elapsed=_el)
                if _upd - _last_log_at['v'] >= min(_LOG_EVERY, _MIN_STEP):
                    _last_log_at['v'] = _upd
                    rate = _upd / max(0.1, _el)
                    remaining = max(0, target_updates - _upd)
                    eta = remaining / max(0.1, rate)
                    wins   = int(getattr(b, 'wins', 0) or 0)
                    losses = int(getattr(b, 'losses', 0) or 0)
                    wr = wins / max(1, wins + losses)
                    buf = len(getattr(b, 'replay', []) or [])
                    print(f"[PreTrainer:{_bname}] {_upd:,} / "
                          f"{target_updates:,} | WR: {wr*100:.1f}% | "
                          f"Rate: {rate:,.0f}/sec | ETA: {eta:.1f}s | "
                          f"Buffer: {buf}", flush=True)
                    _post_training_progress(short_name, attr, _upd,
                                             target_updates, _el)
                _stop.wait(0.5)
        _poller = _ksb_threading.Thread(target=_pump,
                                         name=f'brain-progress-{short_name}',
                                         daemon=True)
        _poller.start()
        try:
            _log_step(short_name, "pretrain.start",
                      f"calling b.pretrain({target_updates:,})")
            b.pretrain(target_updates)
            _log_step(short_name, "pretrain.return", "ok")
            after = _brain_n_updates(attr)
            _elapsed = _ksb_time.time() - _t0
            if hasattr(b, 'save'):
                try:
                    _log_step(short_name, "save.start",
                              f"→ {getattr(b,'dir','?')}")
                    b.save()
                    _log_step(short_name, "save.ok",
                              f"{_ksb_os.path.join(getattr(b,'dir','?'),'model.keras')}")
                except Exception as _se:
                    import traceback
                    print(f"[PreTrainer:{short_name}] SAVE FAILED: {_se}",
                          flush=True)
                    traceback.print_exc()
            phase = 'done' if after >= MIN_ACCEPTABLE_UPDATES else 'error'
            _set_brain_progress(short_name, phase=phase, updates=after,
                                target=target_updates, elapsed=_elapsed,
                                error='' if phase == 'done'
                                else f'only reached {after:,}/{MIN_ACCEPTABLE_UPDATES:,}')
            # Kalshi's PRE-TRAINING COMPLETE block, verbatim shape.
            wins   = int(getattr(b, 'wins', 0) or 0)
            losses = int(getattr(b, 'losses', 0) or 0)
            wr = wins / max(1, wins + losses)
            buf = len(getattr(b, 'replay', []) or [])
            print("=" * 70, flush=True)
            print(f"  PRE-TRAINING COMPLETE [{_bname}] in {_elapsed:.1f}s",
                  flush=True)
            print(f"  Final: {after:,} updates, WR={wr*100:.1f}%, "
                  f"buffer={buf}", flush=True)
            print("=" * 70, flush=True)
            print(f"[PreTrainer:{_bname}] brain saved.\n", flush=True)
            _post_training_progress(short_name, attr, after, target_updates,
                                     _elapsed)
            return phase == 'done'
        except Exception as e:
            import traceback
            _set_brain_progress(short_name, phase='error', error=str(e),
                                elapsed=_ksb_time.time() - _t0)
            print(f"[PreTrainer:{short_name}] TRAIN ERROR: {e}", flush=True)
            traceback.print_exc()
            _log_step(short_name, "pretrain.error", str(e))
            return False
        finally:
            _stop.set()

    def reload_all_brains():
        """Kalshi mirror: after training, reload every roster brain from
        disk so the live process picks up the freshly-trained weights.
        Returns the count reloaded."""
        n = 0
        for short, attr, _sub in BRAIN_ROSTER:
            b = getattr(KSB, attr, None)
            if b is None or not hasattr(b, '_load_or_init'):
                continue
            try:
                b._load_or_init(); n += 1
            except Exception as e:
                print(f"[Brains] reload {short}: {e}")
        return n
    ...
    # Guard so a duplicate call (e.g. after an os.execv restart from the
    # tensorflow variant swap) doesn't double-pretrain or spawn two 24h
    # scheduler threads.
    _ksb_bootstrap_done = False

    def _ksb_ensure_tf_active():
        """KSB imports tensorflow lazily at module load. If TF wasn't
        available then (e.g. _ml_ensure_tf_variant hadn't run yet to
        install the right wheel), _TF_OK is False and every brain's
        pretrain() would silently no-op. Reprobe TF now that the variant
        is settled and, if it's importable, wire keras/layers into the
        KSB module + rebuild each brain's model so pretrain can actually
        create + train the .keras files on disk."""
        try:
            import kalshi_style_brains as _ksb_reload
        except Exception as e:
            print(f"[Brains] KSB module import failed: {e}")
            return
        if _ksb_reload._TF_OK:
            return
        try:
            import tensorflow as _tf_probe  # noqa: F401
            from tensorflow import keras as _tf_keras
            from tensorflow.keras import layers as _tf_layers
        except Exception as e:
            print(f"[Brains] TF still unavailable after variant swap "
                  f"({e}) — brains stay rule-based")
            return
        _ksb_reload._TF_OK = True
        _ksb_reload.keras = _tf_keras
        _ksb_reload.layers = _tf_layers
        for _short, _attr, _sub in BRAIN_ROSTER:
            _b = getattr(_ksb_reload, _attr, None)
            if _b is not None and getattr(_b, 'model', None) is None \
                    and hasattr(_b, '_load_or_init'):
                try: _b._load_or_init()
                except Exception as e:
                    print(f"[Brains] {_attr} model init after "
                          f"TF activation failed: {e}")

    def _brain_status_line():
        """Compact one-line-per-brain summary of current pretrain progress.
        Read live from BRAIN_TRAINING_PROGRESS + n_updates snapshots."""
        rows = []
        snap = brain_training_progress_snapshot()
        for short, attr, _sub in BRAIN_ROSTER:
            p = snap.get(short, {}) or {}
            phase = p.get('phase', 'pending')
            tgt   = max(1, int(p.get('target') or PRETRAIN_TARGET_UPDATES))
            upd   = int(p.get('updates') or _brain_n_updates(attr))
            pct   = min(100, int((upd / tgt) * 100))
            el    = float(p.get('elapsed') or 0.0)
            rate  = upd / max(0.1, el) if el > 0 else 0.0
            rows.append(f"{short}={pct:3d}% ({upd:,}/{tgt:,},{rate:,.0f}/s,"
                        f"{phase})")
        return " │ ".join(rows)

    _status_reporter_started = {'v': False}
    def _start_live_status_reporter(interval_secs=10.0):
        """Background thread: prints a single-line combined brain status
        every N seconds. Prints IMMEDIATELY on first tick (no sleep first)
        so the operator sees the pending row even before pretrain starts.
        Idempotent — a duplicate call is a no-op."""
        if _status_reporter_started['v']:
            return None
        _status_reporter_started['v'] = True
        print(f"[BrainStatus] reporter starting (every {interval_secs:.0f}s)",
              flush=True)
        def _run():
            while True:
                try:
                    line = _brain_status_line()
                    print(f"[BrainStatus] {line}", flush=True)
                    try:
                        if hasattr(KSB, 'TRADING_FLOOR') and hasattr(
                                KSB.TRADING_FLOOR, 'training'):
                            KSB.TRADING_FLOOR.training('BRAIN_STATUS', line)
                    except Exception: pass
                except Exception as e:
                    print(f"[BrainStatus] reporter error: {e}", flush=True)
                # Stop only once every brain has finished (done/built/error).
                try:
                    snap = brain_training_progress_snapshot()
                    if _all_brains_meet_min_floor() and all(
                            (snap.get(s, {}).get('phase')
                             in ('done', 'built', 'error'))
                            for s, _, _ in BRAIN_ROSTER):
                        print("[BrainStatus] all brains done — reporter exit",
                              flush=True)
                        break
                except Exception: pass
                _ksb_time.sleep(interval_secs)
        t = _ksb_threading.Thread(target=_run, name='brain-status-10s',
                                   daemon=True)
        t.start()
        return t

    def _all_brains_meet_min_floor():
        """True iff every roster brain has cumulative n_updates >=
        MIN_ACCEPTABLE_UPDATES. This is the gate main() waits on before
        entering the trading loop."""
        for _, attr, _sub in BRAIN_ROSTER:
            if _brain_n_updates(attr) < MIN_ACCEPTABLE_UPDATES:
                return False
        return True

    def init_and_pretrain_all_brains(
            target_updates=PRETRAIN_TARGET_UPDATES):
        """Kalshi mirror: build every brain, pretrain the ones below the
        target update count, reload from disk, then start engines and the
        24h retrain scheduler. BLOCKS until every roster brain has at
        least MIN_ACCEPTABLE_UPDATES synthetic updates — main() calls this
        before the trading loop so the loop can't start with cold brains.
        Idempotent — the _ksb_bootstrap_done guard makes a duplicate call
        (e.g. from an os.execv restart after a TF variant swap) a no-op."""
        if globals().get('_ksb_bootstrap_done', False):
            _log_step("BOOT", "skip", "already done")
            return
        _current_phase = ["init"]
        try:
            target_updates = max(MIN_ACCEPTABLE_UPDATES,
                                 min(MAX_ACCEPTABLE_UPDATES,
                                     int(target_updates)))
            _log_step("BOOT", "phase0.tf_activate", "reprobing TF")
            _current_phase[0] = "tf_activate"
            _ksb_ensure_tf_active()
            _log_step("BOOT", "phase0.tf_activate.ok",
                      f"TF={_KSB_AVAILABLE}")

            # py-spy dump at brain-training startup — snapshot every
            # thread's live Python stack so the operator sees exactly
            # which threads are running before pretrain begins.
            _current_phase[0] = "py_spy_startup"
            try: _dump_py_spy(tag="brain_training_start")
            except Exception as _pe:
                _log_step("py-spy", "startup.error", str(_pe))

            # Seed KSB with the NASDAQ universe the brains train + score
            # against. Direct port of kalshi's BRAIN_COINS setup.
            _current_phase[0] = "set_brain_symbols"
            try:
                if hasattr(KSB, 'set_brain_symbols'):
                    KSB.set_brain_symbols(NASDAQ_BRAIN_SYMBOLS)
                    _log_step("BOOT", "phase0.symbols",
                              f"{len(NASDAQ_BRAIN_SYMBOLS)} symbols set")
            except Exception as _se:
                _log_step("BOOT", "phase0.symbols.error", str(_se))

            # Kalshi-style startup banner.
            print("═" * 70, flush=True)
            print("  KALSHI-STYLE BRAIN SUITE — NASDAQ EDITION", flush=True)
            print(f"  target: {target_updates:,} synthetic updates per brain",
                  flush=True)
            print(f"  min-acceptable: {MIN_ACCEPTABLE_UPDATES:,} updates "
                  f"(main loop gate)", flush=True)
            print(f"  roster: {[s for s,_,_ in BRAIN_ROSTER]}", flush=True)
            print("═" * 70, flush=True)

            _current_phase[0] = "status_reporter"
            try: _start_live_status_reporter(interval_secs=10.0)
            except Exception as _re:
                _log_step("BOOT", "status_reporter.error", str(_re))

            # ── Phase 1: build every brain up front ──
            _current_phase[0] = "phase1_build"
            _log_step("BOOT", "phase1.build_all_brains", "start")
            _built = build_all_brains()
            _log_step("BOOT", "phase1.build_all_brains.ok",
                      f"built {_built}/{len(BRAIN_ROSTER)}")

            # ── Phase 2: pretrain the brains still below the target floor ──
            _current_phase[0] = "phase2_pretrain"
            had_state = _ksb_brain_state_exists()
            todo = brains_needing_training(target_updates)
            if not todo:
                print(f"[Brains] all {len(BRAIN_ROSTER)} brains already "
                      f"≥{target_updates:,} updates — no pretrain needed",
                      flush=True)
            else:
                if had_state:
                    print(f"[Brains] saved state loaded, {len(todo)} "
                          f"brain(s) need top-up to {target_updates:,}: "
                          f"{todo}", flush=True)
                else:
                    print(f"[Brains] no saved state — pretraining "
                          f"{len(todo)} brain(s) from scratch to "
                          f"{target_updates:,} synth updates each: {todo}",
                          flush=True)
                for short in todo:
                    run_synthetic_pretrainer(short, target_updates)
                # ── Phase 3: reload freshly-trained brains from disk ──
                _reloaded = reload_all_brains()
                print(f"[Brains] reloaded {_reloaded}/{len(BRAIN_ROSTER)} "
                      f"brains after training", flush=True)

            # ── Gate: main loop cannot start until every brain hits the
            # MIN_ACCEPTABLE_UPDATES floor. Retry any laggards up to 3
            # times, then block forever if a brain still can't be trained
            # (better to hang startup with a visible progress bar than to
            # trade live with a cold brain silently returning neutral).
            _gate_attempts = 0
            while not _all_brains_meet_min_floor():
                _gate_attempts += 1
                laggards = [s for s, a, _ in BRAIN_ROSTER
                            if _brain_n_updates(a) < MIN_ACCEPTABLE_UPDATES]
                print(f"[Startup] gate: {len(laggards)} brain(s) below "
                      f"{MIN_ACCEPTABLE_UPDATES:,}-update floor: "
                      f"{laggards} (attempt {_gate_attempts})", flush=True)
                if _gate_attempts > 3:
                    print(f"[Startup] gate: {len(laggards)} brain(s) "
                          f"repeatedly failed to reach the "
                          f"{MIN_ACCEPTABLE_UPDATES:,}-update floor — "
                          f"holding here; check the dashboard progress bars "
                          f"and server logs", flush=True)
                    _ksb_time.sleep(30)
                    continue
                for short in laggards:
                    run_synthetic_pretrainer(short, target_updates)
                reload_all_brains()
            _log_step("BOOT", "gate.cleared",
                      f"all {len(BRAIN_ROSTER)} brains "
                      f"≥{MIN_ACCEPTABLE_UPDATES:,}")

            # ── Phase 4: start the KSB engines DIRECTLY. Do NOT call
            # KSB.start_brain_suite() here — its pretrain_* defaults
            # are 20,000 each and it would re-run KSB's internal pretrain
            # (bypassing our progress bars and hitting the save-dir race
            # user observed as '[Errno 2] No such file or directory:
            # .../REGIME/model.keras'). We already pretrained + reloaded
            # in phases 2-3, so just spin up the background engines.
            _current_phase[0] = "phase4_engines"
            _engines_started = []
            for _ename, _ecls in (
                    ('VolatilityRegimeEngine', 'VolatilityRegimeEngine'),
                    ('RiskSentinelEngine',     'RiskSentinelEngine'),
                    ('CorrelationRiskEngine',  'CorrelationRiskEngine'),
                    ('CapitalAllocationEngine','CapitalAllocationEngine'),
                    ('ProfitGrowthEngine',     'ProfitGrowthEngine'),
            ):
                _cls = getattr(KSB, _ecls, None)
                if _cls is None:
                    _log_step("BOOT", f"engine.{_ename}.missing", "")
                    continue
                try:
                    _log_step("BOOT", f"engine.{_ename}.start", "")
                    _eng = _cls(KSB.BUS)
                    _eng.start()
                    _engines_started.append(_ename)
                    KSB._engines.append(_eng)
                    _log_step("BOOT", f"engine.{_ename}.ok", "")
                except Exception as _ee:
                    import traceback
                    _log_step("BOOT", f"engine.{_ename}.error", str(_ee))
                    traceback.print_exc()
            print(f"[Startup] Brains ready: {len(BRAIN_ROSTER)} live "
                  f"(floor={target_updates:,} updates each), "
                  f"engines={_engines_started}", flush=True)

            # ── Phase 5: 24h background retrain scheduler ──
            _current_phase[0] = "phase5_scheduler"
            _sched_thread = _ksb_threading.Thread(
                target=_ksb_scheduler_loop, name="KSB-24h-retrain",
                daemon=True)
            _sched_thread.start()
            _log_step("BOOT", "phase5.scheduler.ok", "")
            globals()['_ksb_bootstrap_done'] = True
            _log_step("BOOT", "done", "startup init complete")
        except Exception as e:
            _startup_crash_check(e, _current_phase[0])
            try: _dump_py_spy(tag=f"crash.{_current_phase[0]}")
            except Exception: pass

    # Legacy alias kept for any older call sites.
    _ksb_bootstrap_run = init_and_pretrain_all_brains

    # ── First-thing-at-import: create every brain's on-disk directory if
    # missing, so any KSB code path that saves before init_and_pretrain
    # runs (or in parallel with it) can never fail with [Errno 2] No such
    # file or directory. Idempotent — makedirs(exist_ok=True) on each.
    try:
        _ensure_brain_dirs()
        print(f"[Startup] brain directories ready under {KSB.BRAIN_DIR}",
              flush=True)
    except Exception as _de:
        print(f"[Startup] brain-dir precreate failed: {_de}", flush=True)
else:
    def init_and_pretrain_all_brains(target_updates=0):
        """No-op when KSB is unavailable."""
        return
    _ksb_bootstrap_run = init_and_pretrain_all_brains
...
def main():
    ...
    # First thing: make sure every brain's on-disk directory exists so
    # no save path can fail with [Errno 2] No such file or directory.
    # Idempotent — safe to run again after the module-import precreate
    # (and safe to run after an os.execv tensorflow-variant restart).
    try:
        if _KSB_AVAILABLE:
            _ensure_brain_dirs()
            print(f"[Startup] brain directories verified under {KSB.BRAIN_DIR}",
                  flush=True)
    except Exception as _de:
        print(f"[Startup] brain-dir verify failed: {_de}", flush=True)
    ...
    # Start the every-10-seconds combined BrainStatus ticker BEFORE
    # pretrain so it's visible even if pretrain crashes or takes a while
    # to reach its own reporter call. Idempotent.
    if _KSB_AVAILABLE:
        try: _start_live_status_reporter(interval_secs=10.0)
        except Exception as _re:
            print(f"[BrainStatus] launcher failed: {_re}", flush=True)

    # Kalshi-style brain creation + pretrain, called HERE (not at module
    # import) so it runs AFTER _ml_ensure_tf_variant() has installed the
    # correct tensorflow build. Running earlier would silently no-op
    # every brain's pretrain() when TF wasn't importable at first load —
    # the reason brains weren't getting created or trained at startup.
    # Mirrors the kalshi day-trading robot's Console.run() call:
    #     init_and_pretrain_all_brains()  → build → pretrain → reload
    #                                     → start engines → 24h scheduler
    try:
        init_and_pretrain_all_brains()
    except Exception as e:
        logging.warning(f"Brains: init_and_pretrain_all_brains raised: {e}")
