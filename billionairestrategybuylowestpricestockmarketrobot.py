...
if _KSB_AVAILABLE:
    ...
    def run_synthetic_pretrainer(short_name,
        ...
        # Post the initial % to the Brain Comms trading floor so the
        # dashboard's terminal shows training progress in-line with the
        # rest of the brain chatter.
        _post_training_progress(short_name, attr, have, target_updates, 0.0)

        # Side-poller: while pretrain() runs (one blocking model.fit()),
        # sample n_updates every 500ms so the dashboard progress bar moves,
        # print a % line to the terminal each 10% step, and drop a matching
        # % line into the Brain Comms trading-floor transcript.
        ...
        def _pump():
            while not _stop.is_set():
                ...
                pct = int(min(100, (_upd / max(1, target_updates)) * 100))
                bucket = (pct // 10) * 10   # 0,10,20,…,100
                if bucket > _last_pct_shown['v']:
                    _last_pct_shown['v'] = bucket
                    rate = _upd / max(0.1, _el)
                    print(f"[PreTrainer:{short_name}] {pct:3d}% "
                          f"({_upd:,}/{target_updates:,} updates, "
                          f"{rate:,.0f}/s, {_el:.1f}s)", flush=True)
        ...
        try:
            ...
            print(f"[PreTrainer:{short_name}] 100% done in {_elapsed:.1f}s "
                  f"— now at {after:,} updates", flush=True)
