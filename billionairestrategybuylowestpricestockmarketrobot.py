...
if _KSB_AVAILABLE:
    ...
    def _ensure_brain_dirs():
        ...
        exist before pretrain() can save its model.keras. KSB creates them
        in __init__ but that can be skipped or the dir removed later; this
        recreates them so we never hit '[Errno 2] No such file or directory'
        during save."""
        try:
            _ksb_os.makedirs(KSB.BRAIN_DIR, exist_ok=True)
        except Exception as e:
            print(f"[Brains] cannot create KSB.BRAIN_DIR: {e}")
        for _short, attr, sub in BRAIN_ROSTER:
            b = getattr(KSB, attr, None)
            dirs = set()
            if b is not None and getattr(b, 'dir', None):
                dirs.add(b.dir)
            dirs.add(_ksb_os.path.join(KSB.BRAIN_DIR, sub))
            for d in dirs:
                try:
                    _ksb_os.makedirs(d, exist_ok=True)
                except Exception as e:
                    print(f"[Brains] makedirs {d}: {e}")
    ...
    def build_all_brains():
        """Kalshi mirror: construct every roster brain up front so their
        model files exist on disk before any training or live scoring runs.
        Idempotent — each brain's _load_or_init handles the already-built
        case. Returns the count of brains successfully built. Emits
        per-brain progress updates the dashboard reads for its progress bars."""
        _ensure_brain_dirs()
        built = 0
        for short, attr, _sub in BRAIN_ROSTER:
            ...
            if b is None:
                ...
                print(f"[Brains] {attr}: not present in KSB — skipped")
                continue
            try:
                ...
                _set_brain_progress(short, phase='error', error=str(e),
                                    elapsed=_ksb_time.time() - _t0)
                print(f"[Brains] build {short} ({attr}): {e}")
        return built
    ...
    def run_synthetic_pretrainer(short_name,
        ...
        try:
            ...
            _set_brain_progress(short_name, phase='error', error=str(e),
                                elapsed=_ksb_time.time() - _t0)
            print(f"[PreTrainer:{short_name}] error: {e}")
            return False
        finally:
            _stop.set()
    ...
    def init_and_pretrain_all_brains(
        ...
        try:
            if hasattr(KSB, 'set_brain_symbols'):
                ...
                print(f"[Startup] NASDAQ brain symbols: "
                      f"{NASDAQ_BRAIN_SYMBOLS}", flush=True)
        except Exception as _se:
            print(f"[Startup] set_brain_symbols failed: {_se}", flush=True)
        ...
        # Start the every-10-seconds live status ticker before Phase 1
        # so the operator sees combined progress while build+pretrain run.
        try: _start_live_status_reporter(interval_secs=10.0)
        except Exception as _re:
            print(f"[BrainStatus] reporter start failed: {_re}", flush=True)

        # ── Phase 1: build every brain up front ──
        print("[Startup] Building all brains + meta-brains…", flush=True)
        _built = build_all_brains()
        print(f"[Startup] built {_built}/{len(BRAIN_ROSTER)} brains",
              flush=True)
        ...
        try:
            ...
            while not _all_brains_meet_min_floor():
                ...
                for short in laggards:
                    run_synthetic_pretrainer(short, target_updates)
                reload_all_brains()
            print(f"[Startup] gate: all {len(BRAIN_ROSTER)} brains "
                  f"≥{MIN_ACCEPTABLE_UPDATES:,} updates — floor cleared",
                  flush=True)
            ...
            for _ename, _ecls in (
                ...
                try:
                    ...
                    print(f"[Startup] engine {_ename} start failed: {_ee}",
                          flush=True)
            ...
            _sched_thread = _ksb_threading.Thread(
                ...
                daemon=True)
            _sched_thread.start()
            print("[Startup] 24h retrain scheduler live", flush=True)
            globals()['_ksb_bootstrap_done'] = True
        except Exception as e:
            import traceback
            print(f"[Startup] Brain init failed: {e}", flush=True)
            traceback.print_exc()
