...
if _KSB_AVAILABLE:
    ...
    # Mirrors kalshi's PRETRAIN_TARGET_UPDATES: every brain must reach at
    # least this many synthetic updates before entering live decisioning,
    # otherwise it ignores real signals and forces the gate into shadow.
    PRETRAIN_TARGET_UPDATES = 5000
    ...
    def build_all_brains():
        ...
        case. Returns the count of brains successfully built."""
        built = 0
    ...
    def brains_needing_training(target_updates=PRETRAIN_TARGET_UPDATES):
        ...
        cumulative n_updates < target_updates and therefore still needs a
        synthetic pretrain pass. Cheap — reads each brain's snapshot()."""
        todo = []
        for short, attr, _sub in BRAIN_ROSTER:
            if _brain_n_updates(attr) < target_updates:
                todo.append(short)
        return todo
    ...
    def run_synthetic_pretrainer(short_name,
        ...
        brain's own pretrain(n), which handles the synthetic generation and
        model.fit() internally. Returns True on completion, False on error."""
        ...
        need = max(0, target_updates - have)
        if need == 0:
        ...
        try:
            ...
            print(f"[PreTrainer:{short_name}] done in {_elapsed:.1f}s "
                  f"— now at {after:,} updates")
            if hasattr(b, 'save'):
                try: b.save()
    ...
    def init_and_pretrain_all_brains(
        ...
        24h retrain scheduler. Idempotent — the _ksb_bootstrap_done guard
        makes a duplicate call (e.g. from an os.execv restart after a TF
        variant swap) a no-op."""
        if globals().get('_ksb_bootstrap_done', False):
            return
        _ksb_ensure_tf_active()

        # ── Phase 1: build every brain up front ──
        print("[Startup] Building all brains + meta-brains…")
        ...
        try:
            ...
            if not todo:
                ...
                print(f"[Brains] reloaded {_reloaded}/{len(BRAIN_ROSTER)} "
                      f"brains after training")

            # ── Phase 4: start the KSB engines (regime/sentinel/etc.) ──
            # start_engines=True kicks the background threads. Pass
            # pretrain_* = 0 so start_brain_suite doesn't repeat the
            # training we just did in phase 2.
            _zero_kwargs = {f'pretrain_{s}': 0 for s, _, _ in BRAIN_ROSTER}
            KSB.start_brain_suite(**_zero_kwargs, start_engines=True)
            ...
            _sched_thread = _ksb_threading.Thread(
                ...
                daemon=True)
            _sched_thread.start()
            print("[Startup] 24h retrain scheduler live")
            globals()['_ksb_bootstrap_done'] = True
        except Exception as e:
            print(f"[Startup] Brain init failed: {e}")
