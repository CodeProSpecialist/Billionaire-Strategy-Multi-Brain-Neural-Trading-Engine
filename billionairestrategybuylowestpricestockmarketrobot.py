...
if _KSB_AVAILABLE:
    ...
    def _start_live_status_reporter(interval_secs=10.0):
        """Background thread: every N seconds while pretraining is running,
        print a single line summarising every brain's %-complete + rate +
        phase. Stops on its own once every roster brain reaches the min
        floor. Runs alongside the per-brain 5,000-update kalshi log lines."""
        def _run():
            while True:
                try:
                    line = _brain_status_line()
                    print(f"[BrainStatus] {line}", flush=True)
                    # Also drop it onto the Brain Comms trading floor so
                    # the dashboard terminal shows the same ticker.
                    try:
                    ...
                    print(f"[BrainStatus] reporter: {e}", flush=True)
                if _all_brains_meet_min_floor() and all(
                        (brain_training_progress_snapshot().get(s, {}).get(
                            'phase') in ('done', 'built', 'error'))
