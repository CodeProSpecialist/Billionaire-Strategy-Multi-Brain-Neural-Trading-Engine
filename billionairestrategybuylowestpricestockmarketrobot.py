...
if _KSB_AVAILABLE:
    ...
    def _ksb_retrain_all_from_replay():
        ...
        still too small after 24h of live trading. Defers up to 5 min if
        a buy/sell decision cycle is currently in progress."""
        global _KSB_LAST_RETRAIN_TS
        _wait_start = _ksb_time.time()
        while _DECISION_IN_PROGRESS.is_set():
            if _ksb_time.time() - _wait_start > 300:
                print("[KSB 24h] decision window held >5 min, retraining anyway")
                break
            _ksb_time.sleep(2.0)
        try:
